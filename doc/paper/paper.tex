...
\email{paulius.dilkas@nus.edu.sg}}

\maketitle

% TODO: Add a data accessibility statement in the paper.  Data accessibility statements provide information about where the research data and other artefacts supporting the results reported in the paper can be found. Where applicable, links to the repository where the dataset(s) are publicly archived are included.
% TODO: shorten to 15 pages (excluding just references, the acknowledgements must fit)
% TODO: check for compilation errors/warnings (including bibtex)
...
\begin{abstract}
  ...
  \keywords{Weighted model counting \and Random model \and Parameterised
    complexity}
\end{abstract}
...
% related work on SAT (including parameters)
...
% Weights such as zero and one are
% particularly `simplifying' because they are respectively the additive and
% multiplicative identities. Having them propagate through the algorithm reduces
% the size of many ADDs used by \textsc{DPMC}, making the algorithm more
% efficient. Including many copies of the same weight (e.g., 0.5) can similarly
% simplify ADDs as well.

% We expect to observe
% (possibly different) values of $\mu$ that maximize the running time of each
% algorithm for fixed values of $\nu$ and $\kappa$.

% Any \textsf{WMC} algorithm can be made sensitive to determinism by, e.g.,
% preprocessing the instance to eliminate the deterministic variable altogether.
...
\end{algorithm}
% TODO: Q1. The idea behind the random instance generator is to reduce the density of the primal graph (via having some overlapping edges) while:
% a) avoiding having many variables that do not occur in any clause and
% b) promoting tree-like subgraphs that are likely to have low treewidth.
% The final version of the paper will include a definition of treewidth and a better explanation of the intuition behind Algorithm 1.
