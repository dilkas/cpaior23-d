...
//#define LEARNING_OFF		// for turning off clause-learning, added by
// sang
//#define DEBUG_OUT			// output debug information added by
// sang #define CACHE_SIZE			100000 // for hashtable size
#define TOCOMPONENTS   // added by sang
#define LARGEST_DEGREE // decision heuristic
//#define STATIC_ORDERING		// decision heuristic
#define MAX_DISTANCE                                                           \
  0 // minimum distance to a cached component for a component to be cached
//#define CHOOSE_ACTIVE_VAR	// in a decision, only choose active var in the
// residual components #define NON_ACTIVE_STACK	// store non-active vars in a
// stack to avoid future decision on them
//#define DISABLE_CLASUES		// disable clauses that are not in the
// active components #define SORT_CLAUSES_IN_FORMULA
...
// typedef vector<vector <int> *> formula; // define the formula type, added by
// sang
...
literals. They are being "watched". A literal is marked with 2 bits: 00->not
watched; 11->watched, direction = 1;  01->watched, dir = -1; 10 is not valid.
These two bits occupy the least significant bits of the literal.

               Each literal is represented by a 32 bit signed integer. The
higher 29 bits represent the variable index. At most 2**28 varialbes are
               allowed. If the sign of this integer is negative, it means that
it is not a valid literal. It could be a clause index or a deleted literal pool
element. The 3rd least significant bit is used to mark its sign. 0->positive,
1->negative.
...
public:
  ...
  unsigned var_sign(void) { return ((_val >> 2) & 0x1); }
  ...
  void set_clause_index(int cl_idx) { _val = -cl_idx; }
...
analysis. Conflict clauses are generated by a resolution process. Therefore, if
after one search, some clauses got deleted, then some of the learned conflict
clause may be invalidated. To maintain the integrity of the clause database, it
is necessary to keep track of the clauses that are involved in the resolution
process for a certain conflict clause so that when those clauses are deleted,
               the conflict clause should also be deleted.

               The scheme we implement is similar to the scheme described in:
               Ofer Strichman, Pruning techniques for the SAT-based Bounded
Model Checking Problems, in Proc. 11th Advanced Research Working Conference on
...
public:
  ...
  void init(CLitPoolElement *head, unsigned num_lits,
            unsigned gflag) { // initialization of a clause
    _first_lit = head;
    _num_lits = num_lits;
    _gflag = gflag;
  }
  ...
  unsigned &counter_one(void) { return _counter_one; } // added by sang
  ...
  CLitPoolElement *&
  first_lit(void) { // use it only if you want to modify _first_lit
    return _first_lit;
  }

  unsigned &num_lits(void) { return _num_lits; }

  unsigned id(void) { return _id; }

  void set_id(int id) { _id = id; }

  CLAUSE_STATUS status(void) { return _status; }

  void set_status(CLAUSE_STATUS st) { _status = st; }

  // manipulate the group flag
  unsigned &gflag(void) { return _gflag; }
...
public:
  ...
  bool internal;   // a var can be exteral (no parent, or added at some node) or
                   // internal (with parent(s))
...
protected:
  ...
  int _implied_sign : 1; // when a var is implied, here is the sign
  ...
  int _assgn_stack_pos;
  ; // the position where it is in the assignment stack

  int _lits_count[2]; // how many literals are there with this variable. (two
                      // phases)

  vector<CLitPoolElement *>
      _watched[2]; // watched literals of this var. 0: pos phase, 1: neg phase

#ifdef KEEP_LIT_CLAUSES
  vector<ClauseIdx> _lit_clauses[2]; // this will keep track of ALL the
                                     // appearance of the variable in clauses
  // note this will increase the database size by upto a factor of 2
#endif
  int _scores[2]; // the score used for decision making

  int _var_score_pos; // keep track of this variable's position in the sorted
                      // score array
...
public:
  ...
  int score(void) {
    ...
    return score(0) +
           score(1); // changed from max individual score to combined sum score
  }
  ...
  void set_var_score_pos(int pos) { _var_score_pos = pos; }

  unsigned value(void) { return _value; }

  void set_value(unsigned v) { _value = v; }

  // added by sang -- begin

  void set_static_score(int i) { _static_score = i; }

  void set_tried_both(bool t) { _tried_both = t; }

  bool tried_both(void) { return _tried_both; }

  void touched(void) { _touched = true; }

  void untouched(void) { _touched = false; }

  bool is_touched(void) { return _touched; }
  // added by sang -- end

  int &dlevel(void) { return _dlevel; }

  int get_dlevel(void) const { return _dlevel; }

  void set_dlevel(int dl) { _dlevel = dl; }

  int &assgn_stack_pos(void) { return _assgn_stack_pos; }

  int get_assgn_stack_pos(void) const { return _assgn_stack_pos; }

  int &lits_count(int i) { return _lits_count[i]; }

  bool is_marked(void) { return _marked; }

  int get_implied_sign(void) { return _implied_sign; }

  void set_implied_sign(int sign) { _implied_sign = sign; }

  unsigned new_cl_phase(void) { return _new_cl_phase; }

  void set_new_cl_phase(unsigned phase) { _new_cl_phase = phase; }

  void set_marked(void) { _marked = true; }

  void clear_marked(void) { _marked = false; }

  ClauseIdx &antecedent(void) { return _antecedent; }

  ClauseIdx get_antecedent(void) { return _antecedent; }

  void set_antecedent(ClauseIdx cl) { _antecedent = cl; }

  vector<CLitPoolElement *> &watched(int i) { return _watched[i]; }

  void enable_branch(void) { _enable_branch = true; }

  void disable_branch(void) { _enable_branch = false; }

  bool is_branchable(void) { return _enable_branch; }
#ifdef KEEP_LIT_CLAUSES
  vector<ClauseIdx> &lit_clause(int i) { return _lit_clauses[i]; }
#endif
