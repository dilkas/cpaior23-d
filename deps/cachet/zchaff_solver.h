...
enum SAT_DeductionT { CONFLICT, NO_CONFLICT, SAT };
...
struct cmp_var_assgn_pos {
  bool operator()(const CVariable *v1, const CVariable *v2) const {
    if (v1->get_dlevel() > v2->get_dlevel())
      return true;
    else if (v1->get_dlevel() < v2->get_dlevel())
      return false;
    else if (v1->get_assgn_stack_pos() > v2->get_assgn_stack_pos())
      return true;
    return false;
  }
};
...
struct ImplicationQueue : queue<CImplication> {
  void dump(ostream &os) {
    queue<CImplication> temp(*this); // have to make a copy, since queue can
                                     // only be accessed destructively
    os << "Implication Queue Previous: ";
    while (!temp.empty()) {
      CImplication a = temp.front();
      os << "(" << ((a.lit & 1) ? "-" : "+") << (a.lit >> 1) << "@" << a.dlevel
         << ":" << a.antecedent << ")  ";
      temp.pop();
    }
  }
...
typedef list<Component_item *>
    Components; // list of pointers to components, main structure at each level

struct Component_information {
  // bool tag;						//	for debug
  bool changed;                  //	changed due to corss-component implications
  Components::iterator comp_itr; //	pointer to the component associated with
                                 // this information struct
  int num_clause;                //	number of clauses of the component
  int num_cross_implications;    //	#cross_implications that have changed
                                 // this component
  double
      cross_implication_weight; //	the multiplication of weight of all
                                // cross-implications refering to this component
  unsigned ancestor_index; //	index to the Component_information struct of the
                           // component, which generates 	the first
                           // appearance of this component
  int level;               //	the level where this component is created
  bool active;             //	if this component is active
  bool left_branch_done;   //	if the left branch of this component has been
                           // done
#ifdef BIG_NUM
...
#endif
  unsigned num_children; //	number of components generated by instanciating
                         // a var in the current component
  ...
  int largest_degree; //	the degree of s_var (either pos_degree or
                      // neg_degree)
  int largest_sum;
  int static_score;
  // unsigned largest_distance;		//	the largest distance to a cached
  // component
  vector<int>
      *var_set; //	the set for storing all varariables in the component
  vector<int>
      branched_child_list; // branched but uncached childen of a component
  unsigned hash_index;     //	the hash index of this componet
  // unsigned secondary_index;		//	the secondary hash index of this
  // componet
#ifdef APPROXIMATE_HASHING
  unsigned long secondary_index;
#endif
...
struct branch_information {
  unsigned gid;         //	the component number involved with the next level
                        // decision
  int num_new_children; //	the number of newly created components by
                        // branching on the last level component
  int num_children_of_changed;    // children of a changed component which is
                                  // detected at that level
  vector<int> changed_components; // changed component list at that level
};

struct cross_implication // the cross_component_implication involving var vid
                         // created at level
{
  int vid;
  int level;
...
protected:
  // int counter;
  // bool backtrack_tag; // for debug bool set_tag;
  // // for debug bool flag;
  // // for debug bool flag2; int				hit_sizeA[100];
  // int hit_sizeB[100]; int level_total_components[LEVEL_MAX+1]; int
  // level_new_components[LEVEL_MAX+1]; int
  // level_trivial[LEVEL_MAX+1]; int
  // level_conflicts[LEVEL_MAX+1]; int
  // level_hits[LEVEL_MAX+1];
  ...
  vector<cross_implication *>
      cross_implication_list; // list for storing all cross component
                              // implication
  ...
  unsigned bound;
  // int				num_back_track;
  // int				_num_branchable_components;
  // // for decision making
  int backtrack_bound;
  ...
  vector<Component_information *>
      _component_infor_stack; // global array of component information
  // vector <Components *> _components_stack;		// added by sang
  // vector <int>	_gid_stack; // stack of branching components
  vector<branch_information *>
      _branch_infor_stack;       // stack of branch information
  ...
  vector<pair<int, pair<HookFunPtrT, int>>>
      _hooks; // hook function run after certain number of decisions
  OutsideConstraintHookPtrT _outside_constraint_hook;

  // for hash table	added by sang
  bool
  extract_formula_from_clauses(formula &); // get formula of the current level
  // int extract_components_from_clauses(Components &, int);
  int extract_new_components(Components &, int, unsigned);
  int find_component(
      CVariable &, int); // find out the component corresponding to the variable
#ifdef BIG_NUM
  ...
  // int to_components(formula &);	// divide the formula into components,
  // return number of components

  // these are for decision making
  int _max_score_pos; // index the unassigned var with max score
  vector<pair<CVariable *, int>>
      _ordered_vars; // pair's first pointing to the var, second is the score.
  vector<vector<int> *> _non_active_free_var_stack; // added by sang
  bool is_active(
      int); // check if a var is active in the residual formula, added by sang
...
protected:
  ...
  int find_max_clause_dlevel(
      ClauseIdx cl); // the max dlevel of all the assigned lits in this clause
  void back_track(int level);
  void far_back_track(int level); //	added by sang

  // for another way of doing conflict analysis
  void set_up_resolve(ClauseIdx conf_cl,
...
public:
  ...
  // bool			dynamic_heuristic; // true means dynamic largest
  // degree, false means static
  ...
  void add_outside_constraint_hook(OutsideConstraintHookPtrT fun) {
