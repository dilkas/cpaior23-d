...
\renewcommand{\randomizedAlgorithm}[1]{#1} 
...
\title{On the empirical scaling of running time of Lingeling for solving the csmacdp0neg HWMCC08 circuit bounded model checking SAT instances}
...
of the running time of Lingeling for solving the csmacdp0neg HWMCC08 circuit bounded model checking SAT instances.
...
\randomizedAlgorithm{%
Since Lingeling is a randomized algorithm, we analyzed the per-instance
median of 10 independent runs for
...
obtained from our scaling models, based on 51 bootstrap samples
per instance set and 51 automatically fitted variants of each scaling
model.
...
500 instances of different sizes \randomizedAlgorithm{with
10 independent runs per instance}. We split the
running times into two categories, \emph{support} or \emph{training}
instances ($n\leq757881$) and \emph{challenge} or
\emph{test} instances ($n>757881$) with 151
and 349 instances, respectively. The
...
% Lingeling solving the csmacdp0neg HWMCC08 circuit bounded model checking SAT instances.
% \begin{figure*}[tb]
% \begin{centering}
...
of Lingeling, as described in Section \ref{sec:Methodology}. The
...
boldface.}
\end{table}
In addition, we illustrate the fitted models of Lingeling in
...
Lingeling solving the set of the csmacdp0neg HWMCC08 circuit bounded model checking SAT instances
with $200\leq n\leq 757881$ variables, and are challenged by the medians of
the \randomizedAlgorithm{per-instance median}
running times of $757881< n \leq 2545110$ variables.}
\end{figure}
...
the Exp model over-estimates the data, and the Poly model fits the data very well
...
The model with the smallest lower bound is shown in boldface, as well as any models with overlapping intervals. }}
\end{table*}
...
predictions and observed running times on the csmacdp0neg HWMCC08 circuit bounded model checking SAT instances.
...
running time predictions and observed running times on the csmacdp0neg HWMCC08 circuit bounded model checking SAT instances.
...
behaviour of Lingeling on the csmacdp0neg HWMCC08 circuit bounded model checking SAT instances. We found
the Exp model over-estimates the data, and the Poly model fits the data very well.
