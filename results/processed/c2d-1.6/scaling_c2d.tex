...
\title{On the empirical scaling of running time of c2d for solving random instances with mu = 1.6}
...
of the running time of c2d for solving random instances with mu = 1.6.
...
4792 instances of different sizes \randomizedAlgorithm{with
1 independent runs per instance}. We split the
running times into two categories, \emph{support} or \emph{training}
instances ($n\leq22$) and \emph{challenge} or
\emph{test} instances ($n>22$) with 1450
and 3342 instances, respectively. The
...
% c2d solving random instances with mu = 1.6.
% \begin{figure*}[tb]
% \begin{centering}
...
c2d solving the set of random instances with mu = 1.6
with $15\leq n\leq 22$ variables, and are challenged by the medians of
the \randomizedAlgorithm{per-instance median}
running times of $22< n \leq 31$ variables.}
\end{figure}
...
behaviour of c2d on random instances with mu = 1.6. We found
the Exp model tends to under-estimate the data, and the Poly model tends to under-estimate the data.
