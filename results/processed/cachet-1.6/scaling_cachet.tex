...
\title{On the empirical scaling of running time of cachet for solving random instances with mu = 1.6}
...
of the running time of cachet for solving random instances with mu = 1.6.
...
\randomizedAlgorithm{%
Since cachet is a randomized algorithm, we analyzed the per-instance
...
The dataset contains running times of the cachet algorithm solving
...
instances ($n\leq23$) and \emph{challenge} or
\emph{test} instances ($n>23$) with 1914
and 3186 instances, respectively. The
...
% cachet solving random instances with mu = 1.6.
% \begin{figure*}[tb]
% \begin{centering}
...
of cachet, as described in Section \ref{sec:Methodology}. The
...
boldface.}
\end{table}
In addition, we illustrate the fitted models of cachet in
...
cachet solving the set of random instances with mu = 1.6
with $15\leq n\leq 23$ variables, and are challenged by the medians of
the \randomizedAlgorithm{per-instance median}
running times of $23< n \leq 32$ variables.}
\end{figure}
...
predictions and observed running times on random instances with mu = 1.6.
...
running time predictions and observed running times on random instances with mu = 1.6.
...
behaviour of cachet on random instances with mu = 1.6. We found
the Exp model tends to under-estimate the data, and the Poly model tends to under-estimate the data.
