...
D4 = ["../deps/d4"]
QUERY = "../deps/query-dnnf/src/query-dnnf"

parser = argparse.ArgumentParser(
    description="Run c2d/d4 and then query-dnnf to perform WMC inference"
)
parser.add_argument(
    "algorithm",
    choices=["c2d", "d4"],
    metavar="algorithm",
    help="a d-DNNF compilation algorithm (c2d or d4)",
)
...
if args.memory:
    ...
    c2d = subprocess.Popen(
        command,
        preexec_fn=lambda: resource.setrlimit(
            resource.RLIMIT_AS, (int(SOFT_MEMORY_LIMIT * mem), mem)
        ),
    )
    c2d.wait()
    ...
    c2d.wait()
    process = subprocess.Popen(QUERY, stdin=subprocess.PIPE, stdout=subprocess.PIPE)

output, _ = process.communicate(
    "load {}.nnf\nw {}.weights\nmc".format(args.instance, args.instance).encode()
)
process.wait()
