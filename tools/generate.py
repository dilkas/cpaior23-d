...
class InstanceGenerator:
    ...
    def __init__(self, num_variables, clause_factor, literal_factor,
                 repetitiveness, prop_deterministic, prop_equal, kcnf):
    ...
    def random_partition(self, total, count):
        ...
        while True:
            ...
            partition = [bars[0]] + [bars[i] - bars[i-1] for i in range(1, len(bars))] + [total - bars[-1]]
    ...
    def new_literal(self, previous_variables, primal_graph):
        ...
        denominator = sum(len(primal_graph[v - 1] - set(previous_variables)) for v in previous_variables)
        choices = list(set(range(1, self.num_variables + 1)) - set(previous_variables))
        if denominator == 0:
            return random.choice(choices)

        # TODO: is there a way to not recreate the weights?
        # Or maybe just have a one weight for all the equal probabilities?
        weights = [(1 - self.repetitiveness) / (self.num_variables - len(previous_variables))] * len(choices)
        for i in range(len(choices)):
            weights[i] += (self.repetitiveness * len(
                primal_graph[choices[i] - 1] & set(previous_variables)) / denominator)
        if VERBOSE_MODE:
    ...
    def generate_instance(self):
        literals_per_clause = self.random_partition(int(self.num_variables *
                                                        self.clause_factor *
                                                        self.literal_factor),
    ...
    def output_graph(self, clauses):
        edges = set((min(abs(clause[i]), abs(clause[j])), max(abs(clause[i]), abs(clause[j])))
                    for clause in clauses for i in range(len(clause) - 1)
    ...
    def generate_weights(self):
        ...
        for i in range(first_equal):
            ...
            if VERBOSE_MODE:
                print('Assigning a deterministic weight', weights[variables[i]], 'to variable', variables[i])
        ...
        for i in range(first_probabilistic, len(variables)):
            ...
            if VERBOSE_MODE:
                print('Assigning a random weight', weights[variables[i]], 'to variable', variables[i])
    ...
    def output_minic2d(self, clauses, weights):
        ...
        for variable in range(1, self.num_variables + 1):
            if weights[variable] == -1:
                ...
                weights_line += ' ' + str(weights[variable]) + ' ' + str(1 - weights[variable])
        return s + weights_line + '\n'

    def __str__(self):
        return '-'.join([str(s) for s in [self.num_variables, self.clause_factor,
                                          self.literal_factor, self.repetitiveness,
                                          self.prop_deterministic, self.prop_equal, self.instance_count]])
